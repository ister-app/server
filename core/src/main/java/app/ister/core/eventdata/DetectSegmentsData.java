 ...
 * the season's files, or on a helper node listing that directory (app.ister.helper.disks), which
 * reads the audio over HTTP. Season-level (not per-file) because intros are found by comparing sibling
