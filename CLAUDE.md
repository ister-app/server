...
- `EventType` (in `database/.../enums/EventType.java`) is the logical message type. `Handle.handles()` returns an `EventType`, and `listener()` rejects a message whose `eventType` doesn't match. This is the source of truth for "what kinds of events exist" (33 values).
...
- **Never `Path.of(entity.getPath())` in a handler.** Go through the seams: `ObjectStoreRegistry`
  ...
  PDFBox), `MediaFileInputResolver.resolve(mf)` for every ffmpeg/ffprobe input.
...
- **Bitmap subtitles (PGS/VobSub) are never burned in or OCR'd:** `HlsBitmapSubtitleService` copies the packets out (one ffmpeg run for all streams, nothing decoded) and the pure-Java parsers in `transcoder/.../bitmapsub/` turn them into `bsub_{streamId}.json` + PNG sprite sheets in the tmp dir, lazily (`GET /hls/{id}/bsub_…`, warmed at playlist pre-generation). The player draws them in its own overlay. Cue times are raw (no `SUBTITLE_OFFSET_MS`). `PngEncoder` is hand-written on purpose: no AWT/ImageIO in the native image.
...
`AppIsterServerConfig` (disk), `WorkerDiskConfig`, `OwnDirectoriesProperties`/`HelperProperties`/`TranscoderDisksConfig` (core, deprecated alias),
