            ...
            for (String language : supportLanguages) {
                ...
                if (tmdbResult.isPresent()) {
                    ...
                    if (tmdbResult.get().getBackgroundUrl() != null) {
                        imageDownloadService.downloadAndSave(tmdbResult.get().getBackgroundUrl(), ImageType.BACKGROUND, tmdbResult.get().getLanguage(), "TMDB://" + tmdbResult.get().getBackgroundUrl(), null, null, episodeEntity, null, null);
                    }
                    if (tmdbResult.get().getPosterUrl() != null) {
                        imageDownloadService.downloadAndSave(tmdbResult.get().getPosterUrl(), ImageType.COVER, tmdbResult.get().getLanguage(), "TMDB://" + tmdbResult.get().getPosterUrl(), null, null, episodeEntity, null, null);
                    }
                }
            }
